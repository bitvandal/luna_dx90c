...
pub struct D3DApp {
    ...
    proj_tex_demo: Option<ProjTexDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let proj_tex_demo = ProjTexDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.proj_tex_demo = proj_tex_demo;
                ...
                if let Some(proj_tex_demo) = &d3d_app.proj_tex_demo {
                    proj_tex_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(proj_tex_demo) = &mut self.proj_tex_demo {
            proj_tex_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Projective Tex Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(proj_tex_demo) = &mut self.proj_tex_demo {
                            proj_tex_demo.update_scene(dt);
                            proj_tex_demo.draw_scene();
                        }
