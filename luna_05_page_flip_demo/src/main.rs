    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let page_flip_demo = PageFlipDemo::new(d3d_device.clone());
                d3d_app.page_flip = page_flip_demo;
                ...
                if let Some(sprite_demo) = &d3d_app.page_flip {
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(sprite_demo) = &self.page_flip {
            ...
            match &self.d3d_object {
                ...
                Some(d3d_object) => {
                    ...
                    if (caps.DevCaps & (D3DDEVCAPS_PUREDEVICE as u32) != 0) &&
                        (self.requested_vp != D3DCREATE_SOFTWARE_VERTEXPROCESSING) {
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(sprite_demo) = &mut self.page_flip {
