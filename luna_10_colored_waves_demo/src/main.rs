...
pub struct D3DApp {
    ...
    colored_waves_demo: Option<ColoredWavesDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let colored_waves_demo = ColoredWavesDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.colored_waves_demo = colored_waves_demo;
                ...
                if let Some(colored_waves_demo) = &d3d_app.colored_waves_demo {
                    colored_waves_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(colored_waves_demo) = &self.colored_waves_demo {
            colored_waves_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(colored_waves_demo) = &mut self.colored_waves_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Colored Waves Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(colored_waves_demo) = &mut self.colored_waves_demo {
                            colored_waves_demo.update_scene(dt);
                            colored_waves_demo.draw_scene();
                        }
