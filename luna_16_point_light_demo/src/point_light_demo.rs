...
impl PointLightDemo {
    pub fn new(d3d_device: IDirect3DDevice9, d3d_pp: *const D3DPRESENT_PARAMETERS) -> Option<PointLightDemo> {
        if !PointLightDemo::check_device_caps() {
        ...
        let light_pos_w = D3DXVECTOR3 { x: 0.0, y: 3.0, z: 0.0 };
        ...
        let (vb, ib) = PointLightDemo::build_geo_buffers(d3d_device.clone());

        let (fx, h_tech, h_wvp, h_world_inverse_transpose,
            h_eye_pos, h_world,
            h_ambient_light, h_diffuse_light, h_specular_light,
            h_light_pos_w, h_attenuation012,
            h_ambient_mtrl, h_diffuse_mtrl, h_specular_mtrl, h_specular_power) =
            PointLightDemo::build_fx(d3d_device.clone());
        ...
        let mut point_light_demo = PointLightDemo {
            d3d_pp,
            gfx_stats,
        ...
        point_light_demo.on_reset_device();

        Some(point_light_demo)
    }
    ...
    pub fn update_scene(&mut self, dt: f32) {
        ...
        unsafe {
            ...
            // doesn't get too big.
            if THETA >= 2.0 * D3DX_PI {
                THETA = 0.0;
            }

            self.light_pos_w.z = 25.0 * THETA.sin();
        }
    }
    ...
    pub fn draw_scene(&mut self) {
        unsafe {
            if let Some(d3d_device) = &D3D_DEVICE {
                ...
                HR!(ID3DXBaseEffect_SetValue(self.fx, self.h_light_pos_w, &self.light_pos_w as *const _ as _, std::mem::size_of::<D3DXVECTOR3>() as u32));
    ...
    fn build_fx(d3d_device: IDirect3DDevice9) -> (LPD3DXEFFECT, D3DXHANDLE, D3DXHANDLE, D3DXHANDLE,
        ...
        HR!(D3DXCreateEffectFromFile(d3d_device, PSTR(b"luna_16_point_light_demo/pointlight.fx\0".as_ptr() as _),
        ...
        // Obtain handles.
        let h_tech = ID3DXBaseEffect_GetTechniqueByName(fx, PSTR(b"PointLightTech\0".as_ptr() as _));
        ...
        (fx, h_tech, h_wvp, h_world_inverse_transpose, h_eye_pos, h_world,
         h_ambient_light, h_diffuse_light, h_specular_light,
         h_light_pos_w, h_attenuation012,
         h_ambient_mtrl, h_diffuse_mtrl, h_specular_mtrl, h_specular_power)
    }
