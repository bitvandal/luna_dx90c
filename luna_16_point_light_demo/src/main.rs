...
pub struct D3DApp {
    ...
    point_light_demo: Option<PointLightDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let point_light_demo = PointLightDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.point_light_demo = point_light_demo;
                ...
                if let Some(point_light_demo) = &d3d_app.point_light_demo {
                    point_light_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(point_light_demo) = &self.point_light_demo {
            point_light_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(point_light_demo) = &mut self.point_light_demo {
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(point_light_demo) = &mut self.point_light_demo {
                            point_light_demo.update_scene(dt);
                            point_light_demo.draw_scene();
                        }
