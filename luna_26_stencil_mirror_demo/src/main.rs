...
pub struct D3DApp {
    ...
    stencil_mirror_demo: Option<StencilMirrorDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let stencil_mirror_demo = StencilMirrorDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.stencil_mirror_demo = stencil_mirror_demo;
                ...
                if let Some(stencil_mirror_demo) = &d3d_app.stencil_mirror_demo {
                    stencil_mirror_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(stencil_mirror_demo) = &self.stencil_mirror_demo {
            stencil_mirror_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(stencil_mirror_demo) = &mut self.stencil_mirror_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Mirror Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(stencil_mirror_demo) = &mut self.stencil_mirror_demo {
                            stencil_mirror_demo.update_scene(dt);
                            stencil_mirror_demo.draw_scene();
                        }
