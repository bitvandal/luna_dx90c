...
impl SprinklerPSystem {
    pub fn new(base_path: &str, fx_name: &str, tech_name: &str, tex_name: &str,
               ...
               rng: ThreadRng) -> SprinklerPSystem {
    ...
    pub fn init_particle(&mut self, out: &mut Particle) {
        ...
        out.initial_pos = D3DXVECTOR3 { x: 0.0, y: 0.0, z: 0.0 };

        out.initial_time = self.time;
        out.life_time = get_random_float(&mut self.rng, 4.0, 5.0);
        out.initial_color = D3DCOLOR_XRGB!(255, 255, 255);
        out.initial_size = get_random_float(&mut self.rng, 8.0, 12.0);
        out.mass = get_random_float(&mut self.rng, 0.8, 1.2);

        out.initial_velocity.x = get_random_float(&mut self.rng, -2.5, 2.5);
        out.initial_velocity.y = get_random_float(&mut self.rng, 15.0, 25.0);
        out.initial_velocity.z = get_random_float(&mut self.rng, -2.5, 2.5);
    }
