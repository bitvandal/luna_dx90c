    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let props_demo = PropsDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.props_demo = props_demo;
                ...
                if let Some(props_demo) = &d3d_app.props_demo {
                    props_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(props_demo) = &self.props_demo {
            props_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(props_demo) = &mut self.props_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Props Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(props_demo) = &mut self.props_demo {
                            props_demo.update_scene(dt);
                            props_demo.draw_scene();
                        }
