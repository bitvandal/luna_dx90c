...
pub struct D3DApp {
    ...
    spotlight_demo: Option<SpotlightDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let spotlight_demo = SpotlightDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.spotlight_demo = spotlight_demo;
                ...
                if let Some(spotlight_demo) = &d3d_app.spotlight_demo {
                    spotlight_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(spotlight_demo) = &self.spotlight_demo {
            spotlight_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(spotlight_demo) = &mut self.spotlight_demo {
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(spotlight_demo) = &mut self.spotlight_demo {
                            spotlight_demo.update_scene(dt);
                            spotlight_demo.draw_scene();
                        }
