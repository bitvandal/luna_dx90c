...
pub struct D3DApp {
    ...
    terrain_demo: Option<TerrainDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let terrain_demo = TerrainDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.terrain_demo = terrain_demo;
                ...
                if let Some(terrain_demo) = &d3d_app.terrain_demo {
                    terrain_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(terrain_demo) = &self.terrain_demo {
            terrain_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(terrain_demo) = &mut self.terrain_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Terrain Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(terrain_demo) = &mut self.terrain_demo {
                            terrain_demo.update_scene(dt);
                            terrain_demo.draw_scene();
                        }
