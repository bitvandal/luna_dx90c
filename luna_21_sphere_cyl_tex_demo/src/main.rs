...
pub struct D3DApp {
    ...
    sphere_cyl_tex_demo: Option<SphereCylTexDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let sphere_cyl_tex_demo = SphereCylTexDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.sphere_cyl_tex_demo = sphere_cyl_tex_demo;
                ...
                if let Some(sphere_cyl_tex_demo) = &d3d_app.sphere_cyl_tex_demo {
                    sphere_cyl_tex_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(sphere_cyl_tex_demo) = &self.sphere_cyl_tex_demo {
            sphere_cyl_tex_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(sphere_cyl_tex_demo) = &mut self.sphere_cyl_tex_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Sphere-Cyl-Tex Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(sphere_cyl_tex_demo) = &mut self.sphere_cyl_tex_demo {
                            sphere_cyl_tex_demo.update_scene(dt);
                            sphere_cyl_tex_demo.draw_scene();
                        }
