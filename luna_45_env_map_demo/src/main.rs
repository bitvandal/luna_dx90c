...
pub struct D3DApp {
    ...
    env_map_demo: Option<EnvMapDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let env_map_demo = EnvMapDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.env_map_demo = env_map_demo;
                ...
                if let Some(env_map_demo) = &d3d_app.env_map_demo {
                    env_map_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(env_map_demo) = &mut self.env_map_demo {
            env_map_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Environment Map Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(env_map_demo) = &mut self.env_map_demo {
                            env_map_demo.update_scene(dt);
                            env_map_demo.draw_scene();
                        }
