...
pub mod gfx_stats;
pub mod ambient_diffuse_specular_demo;
pub mod utils;
pub mod vertex;
...
use crate::ambient_diffuse_specular_demo::*;
...
pub struct D3DApp {
    ...
    ambient_diffuse_specular_demo: Option<AmbientDiffuseSpecularDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let ambient_diffuse_specular_demo = AmbientDiffuseSpecularDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.ambient_diffuse_specular_demo = ambient_diffuse_specular_demo;
                ...
                if let Some(ambient_diffuse_specular_demo) = &d3d_app.ambient_diffuse_specular_demo {
                    ambient_diffuse_specular_demo.release_com_objects();
                }
...
impl D3DApp {
    fn new(device_type: D3DDEVTYPE, requested_vp: i32) -> Option<D3DApp> {
        unsafe {
            let mut d3d_app = D3DApp {
                ...
                ambient_diffuse_specular_demo: None,
            };
    ...
    fn on_lost_device(&self) {
        if let Some(ambient_diffuse_specular_demo) = &self.ambient_diffuse_specular_demo {
            ambient_diffuse_specular_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(ambient_diffuse_specular_demo) = &mut self.ambient_diffuse_specular_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Ambient, Diffuse, Spec Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(ambient_diffuse_specular_demo) = &mut self.ambient_diffuse_specular_demo {
                            ambient_diffuse_specular_demo.update_scene(dt);
                            ambient_diffuse_specular_demo.draw_scene();
                        }
