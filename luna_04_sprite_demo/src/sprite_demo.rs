...
impl SpriteDemo {
    pub fn new(d3d_device: IDirect3DDevice9, options: SpriteDemoOptions) -> Option<SpriteDemo> {
        ...
        HR!(D3DXCreateTextureFromFile(d3d_device.clone(), PSTR(b"luna_04_sprite_demo/bkgd1.bmp\0".as_ptr() as _), &mut bkgd_tex));

        let mut ship_tex: *mut c_void = std::ptr::null_mut();
        HR!(D3DXCreateTextureFromFile(d3d_device.clone(), PSTR(b"luna_04_sprite_demo/alienship.bmp\0".as_ptr() as _), &mut ship_tex));

        let mut bullet_tex: *mut c_void = std::ptr::null_mut();
        HR!(D3DXCreateTextureFromFile(d3d_device.clone(), PSTR(b"luna_04_sprite_demo/bullet.bmp\0".as_ptr() as _), &mut bullet_tex));
