...
use crate::*;
use crate::fire_ring_psystem::FireRingPSystem;
use crate::terrain::Terrain;

pub const BASE_PATH: &str = "luna_39_fire_ring_demo/";

// Colors
pub const WHITE: D3DXCOLOR = D3DXCOLOR { r: 1.0, g: 1.0, b: 1.0, a: 1.0 };

// Sample demo
pub struct FireRingDemo {
    d3d_pp: *const D3DPRESENT_PARAMETERS,
    gfx_stats: Option<GfxStats>,

    terrain: Terrain,
    psys: FireRingPSystem,
}

impl FireRingDemo {
    pub fn new(hwnd: HWND, d3d_device: IDirect3DDevice9, d3d_pp: *const D3DPRESENT_PARAMETERS, rng: ThreadRng) -> Option<FireRingDemo> {
        if !FireRingDemo::check_device_caps() {
        ...
        D3DXMatrixTranslation(&mut psys_world, 55.0, 45.0, 55.0);

        let mut psys_box = AABB::default();
        psys_box.min_pt = D3DXVECTOR3 { x: -15.0, y: -15.0, z: -15.0 };
        psys_box.max_pt = D3DXVECTOR3 { x:  15.0, y:  15.0, z:  15.0 };

        let mut psys =
            FireRingPSystem::new(BASE_PATH, "firering.fx", "FireRingTech", "torch.dds",
                                 &D3DXVECTOR3 { x: 0.0, y: 0.9, z: 0.0 }, &psys_box,
                                 1000, 0.0025, hwnd, d3d_device.clone(), rng);
        ...
        let mut fire_ring_demo = FireRingDemo {
            d3d_pp,
            gfx_stats,
