...
impl FireRingPSystem {
    ...
    pub fn init_particle(&mut self, out: &mut Particle) {
        ...
        out.life_time = get_random_float(&mut self.rng, 2.0, 4.0);

        // Initial size in pixels.
        out.initial_size = get_random_float(&mut self.rng, 10.0, 15.0);

        // Give a very small initial velocity to give the flares
        // some randomness.
        get_random_vec(&mut self.rng, &mut out.initial_velocity);

        // Scalar value used in vertex shader as an amplitude factor.
        out.mass = get_random_float(&mut self.rng, 1.0, 2.0);

        // Start color at 50-100% intensity when born for variation.
        let transformed_color: D3DXCOLOR = WHITE.mult(get_random_float(&mut self.rng, 0.5, 1.0));
        out.initial_color = D3DCOLOR_RGBA!((transformed_color.r * 255.0) as u32, (transformed_color.g * 255.0) as u32,
            (transformed_color.b * 255.0) as u32, 1);

        // Generate random particle on the ring in polar coordinates:
        // random radius and random angle.
        let r: f32 = get_random_float(&mut self.rng, 10.0, 14.0);
        let t: f32 = get_random_float(&mut self.rng, 0.0, 2.0 * D3DX_PI);

        // Convert to Cartesian coordinates.
        out.initial_pos.x = r * t.cos();
        out.initial_pos.y = r * t.sin();

        // Random depth value in [-1, 1] (depth of the ring)
        out.initial_pos.z = get_random_float(&mut self.rng, -1.0, 1.0);
    }
