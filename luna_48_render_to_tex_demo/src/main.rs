...
pub struct D3DApp {
    ...
    render_to_tex_demo: Option<RenderToTexDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let d3d_object = &d3d_app.d3d_object.as_ref().expect("D3D init error");
                let render_to_tex_demo =
                    RenderToTexDemo::new(d3d_device.clone(),
                                         &d3d_object,
                                         d3d_app.device_type,
                                         &d3d_app.d3d_pp);
                d3d_app.render_to_tex_demo = render_to_tex_demo;
                ...
                if let Some(render_to_tex_demo) = &d3d_app.render_to_tex_demo {
                    render_to_tex_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(render_to_tex_demo) = &mut self.render_to_tex_demo {
            render_to_tex_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"RenderToTex Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(render_to_tex_demo) = &mut self.render_to_tex_demo {
                            render_to_tex_demo.update_scene(dt);
                            render_to_tex_demo.draw_scene();
                        }
