...
impl StencilShadowDemo {
    pub fn new(d3d_device: IDirect3DDevice9, d3d_pp: *const D3DPRESENT_PARAMETERS) -> Option<StencilShadowDemo> {
        if !StencilShadowDemo::check_device_caps() {
        ...
        let light_vec_w = D3DXVECTOR3 { x: -0.577, y: 0.577, z: -0.577 };
        ...
        let shadow_mtrl = Mtrl {
            ambient: BLACK,
            diffuse: D3DXCOLOR { r: 0.0, g: 0.0, b: 0.0, a: 0.5 },  // 50% transparent.
            spec: BLACK,
            spec_power: 1.0
        };
        ...
        if let Some(gfx_stats) = &mut gfx_stats {
            ...
            gfx_stats.add_vertices(ID3DXBaseMesh_GetNumVertices(teapot) * 3);
            gfx_stats.add_triangles(ID3DXBaseMesh_GetNumFaces(teapot) * 3);
        }

        let room_vb = StencilShadowDemo::build_room_geometry(d3d_device.clone());
        ...
        let (fx,
            ...
            h_world,
            h_tex) =
            StencilShadowDemo::build_fx(d3d_device.clone());

        let mut stencil_shadow_demo = StencilShadowDemo {
            d3d_pp,
            gfx_stats,
        ...
        stencil_shadow_demo.on_reset_device();

        Some(stencil_shadow_demo)
    }
        ...
        unsafe {
            if let Some(dinput) = &mut DIRECT_INPUT {
                ...
                if dinput.key_down(DIK_A as usize) {
                    let mut trans = std::mem::zeroed();
                    D3DXMatrixTranslation(&mut trans, -2.0 * dt, 0.0, 0.0);
                    D3DXMatrixMultiply(&mut self.teapot_world, &self.teapot_world, &trans);
                }
    ...
    pub fn draw_scene(&mut self) {
        unsafe {
            if let Some(d3d_device) = &D3D_DEVICE {
                ...
                HR!(ID3DXBaseEffect_SetValue(self.fx, self.h_ambient_mtrl, &self.shadow_mtrl.ambient as *const _ as _, std::mem::size_of::<D3DXCOLOR>() as u32));
                HR!(ID3DXBaseEffect_SetValue(self.fx, self.h_diffuse_mtrl, &self.shadow_mtrl.diffuse as *const _ as _, std::mem::size_of::<D3DXCOLOR>() as u32));
                HR!(ID3DXBaseEffect_SetValue(self.fx, self.h_specular_mtrl, &self.shadow_mtrl.spec as *const _ as _, std::mem::size_of::<D3DXCOLOR>() as u32));
                HR!(ID3DXBaseEffect_SetFloat(self.fx, self.h_specular_power, self.shadow_mtrl.spec_power));

                self.draw_teapot_shadow(d3d_device.clone());
    ...
    fn draw_teapot_shadow(&mut self, d3d_device: IDirect3DDevice9) {
        unsafe {
            HR!(d3d_device.SetRenderState(D3DRS_STENCILENABLE, 1));
            HR!(d3d_device.SetRenderState(D3DRS_STENCILFUNC, D3DCMP_EQUAL.0 as u32));
            HR!(d3d_device.SetRenderState(D3DRS_STENCILREF, 0x0));
            HR!(d3d_device.SetRenderState(D3DRS_STENCILMASK, 0xffffffff));
            HR!(d3d_device.SetRenderState(D3DRS_STENCILWRITEMASK, 0xffffffff));
            HR!(d3d_device.SetRenderState(D3DRS_STENCILZFAIL, D3DSTENCILOP_KEEP.0));
            HR!(d3d_device.SetRenderState(D3DRS_STENCILFAIL, D3DSTENCILOP_KEEP.0));
            HR!(d3d_device.SetRenderState(D3DRS_STENCILPASS, D3DSTENCILOP_INCR.0));

            // Position shadow.
            let light_direction = D3DXVECTOR4 { x: 0.577, y: -0.577, z: 0.577, w: 0.0 };
            let ground_plane = D3DXPLANE { a: 0.0, b: -1.0, c: 0.0, d: 0.0 };

            let mut s: D3DXMATRIX = std::mem::zeroed();
            D3DXMatrixShadow(&mut s, &light_direction, &ground_plane);

            // Offset the shadow up slightly so that there is no
            // z-fighting with the shadow and ground.
            let mut eps: D3DXMATRIX = std::mem::zeroed();
            D3DXMatrixTranslation(&mut eps, 0.0, 0.001, 0.0);
            ...
            let mut res: D3DXMATRIX = std::mem::zeroed();
            D3DXMatrixMultiply(&mut res, &self.teapot_world, &s);
            D3DXMatrixMultiply(&mut res, &res, &eps);
            self.teapot_world = res;
