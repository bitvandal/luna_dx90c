...
pub struct D3DApp {
    ...
    solar_system_demo: Option<SolarSystemDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let solar_system_demo = SolarSystemDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.solar_system_demo = solar_system_demo;
                ...
                if let Some(solar_system_demo) = &d3d_app.solar_system_demo {
                    solar_system_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(solar_system_demo) = &self.solar_system_demo {
            solar_system_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(solar_system_demo) = &mut self.solar_system_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Solar System Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(solar_system_demo) = &mut self.solar_system_demo {
                            solar_system_demo.update_scene(dt);
                            solar_system_demo.draw_scene();
                        }
