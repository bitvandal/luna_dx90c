...
use crate::diffuse_pyramid_demo::*;
...
pub struct D3DApp {
    ...
    diffuse_pyramid_demo: Option<DiffusePyramidDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let diffuse_pyramid_demo = DiffusePyramidDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.diffuse_pyramid_demo = diffuse_pyramid_demo;
                ...
                if let Some(diffuse_pyramid_demo) = &d3d_app.diffuse_pyramid_demo {
                    diffuse_pyramid_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(diffuse_pyramid_demo) = &self.diffuse_pyramid_demo {
            diffuse_pyramid_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(diffuse_pyramid_demo) = &mut self.diffuse_pyramid_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Diffuse Pyramid Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(diffuse_pyramid_demo) = &mut self.diffuse_pyramid_demo {
                            diffuse_pyramid_demo.update_scene(dt);
                            diffuse_pyramid_demo.draw_scene();
                        }
