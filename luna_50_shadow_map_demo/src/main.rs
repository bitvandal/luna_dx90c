...
pub struct D3DApp {
    ...
    shadow_map_demo: Option<ShadowMapDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                ...
                let shadow_map_demo =
                    ShadowMapDemo::new(d3d_device.clone(),
                                       &d3d_object,
                                       d3d_app.device_type,
                                       &d3d_app.d3d_pp);
                d3d_app.shadow_map_demo = shadow_map_demo;
                ...
                if let Some(shadow_map_demo) = &d3d_app.shadow_map_demo {
                    shadow_map_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(shadow_map_demo) = &mut self.shadow_map_demo {
            shadow_map_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Shadow Map Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(shadow_map_demo) = &mut self.shadow_map_demo {
                            shadow_map_demo.update_scene(dt);
                            shadow_map_demo.draw_scene();
                        }
