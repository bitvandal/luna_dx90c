pub mod camera;
pub mod displacement_map_demo;
pub mod drawable_tex2d;
pub mod sky;
pub mod terrain;
pub mod water_dmap;
...
use crate::displacement_map_demo::DisplacementMapDemo;

use crate::water_dmap::*;
...
pub struct D3DApp {
    ...
    displacement_map_demo: Option<DisplacementMapDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let displacement_map_demo =
                    DisplacementMapDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.displacement_map_demo = displacement_map_demo;
                ...
                if let Some(displacement_map_demo) = &d3d_app.displacement_map_demo {
                    displacement_map_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(displacement_map_demo) = &mut self.displacement_map_demo {
            displacement_map_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Displacement Map Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(displacement_map_demo) = &mut self.displacement_map_demo {
                            displacement_map_demo.update_scene(dt);
                            displacement_map_demo.draw_scene();
                        }
