    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let water_demo = WaterDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.water_demo = water_demo;
                ...
                if let Some(water_demo) = &d3d_app.water_demo {
                    water_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(water_demo) = &mut self.water_demo {
            water_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Water Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(water_demo) = &mut self.water_demo {
                            water_demo.update_scene(dt);
                            water_demo.draw_scene();
                        }
