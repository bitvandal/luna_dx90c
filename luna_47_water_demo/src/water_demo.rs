...
use crate::*;
use crate::sky::Sky;
use crate::water::{Water, WaterInitInfo};

pub const BASE_PATH: &str = "luna_47_water_demo/";
...
impl WaterDemo {
    pub fn new(d3d_device: IDirect3DDevice9, d3d_pp: *const D3DPRESENT_PARAMETERS) -> Option<WaterDemo> {
        unsafe {
            if !WaterDemo::check_device_caps() {
                display_error_then_quit("checkDeviceCaps() Failed");
            }

            init_all_vertex_declarations(d3d_device.clone());

            let mut light_dir_w = D3DXVECTOR3 { x: 0.0, y: -2.0, z: -1.0 };
            ...
            D3DXMatrixTranslation(&mut water_world, 0.0, 2.0, 0.0);

            let water_mtrl = Mtrl {
                ambient: D3DXCOLOR { r: 0.26, g: 0.23, b: 0.3, a: 0.90 },
                diffuse: D3DXCOLOR { r: 0.26, g: 0.23, b: 0.3, a: 0.90 },
                spec: WHITE.mult(1.0),
                spec_power: 64.0,
            };

            let water_init_info = WaterInitInfo {
                dir_light: light.clone(),
                mtrl: water_mtrl,
                vert_rows: 128,
                vert_cols: 128,
                dx: 1.0,
                ...
                wave_map_velocity0: D3DXVECTOR2 { x: 0.05, y: 0.08 },
                wave_map_velocity1: D3DXVECTOR2 { x: -0.02, y: 0.1 },
                tex_scale: 16.0,
                to_world: water_world
            };

            let water = Water::new(BASE_PATH, water_init_info, d3d_device.clone());
            water.set_env_map(sky.get_env_map());
            ...
            if let Some(camera) = &mut CAMERA {
                camera.set_pos(D3DXVECTOR3 { x: 0.0, y: 7.0, z: -30.0 });
            ...
            let (fx,
                ...
                h_light,
                h_normal_map)
                = WaterDemo::build_fx(d3d_device.clone());
