...
pub struct D3DApp {
    ...
    tri_pick_demo: Option<TriPickDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let tri_pick_demo =
                    TriPickDemo::new(d3d_app.get_main_wnd(),
                                     d3d_device.clone(),
                                     &d3d_app.d3d_pp);
                d3d_app.tri_pick_demo = tri_pick_demo;
                ...
                if let Some(tri_pick_demo) = &d3d_app.tri_pick_demo {
                    tri_pick_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(tri_pick_demo) = &mut self.tri_pick_demo {
            tri_pick_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Tri-Pick Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(tri_pick_demo) = &mut self.tri_pick_demo {
                            tri_pick_demo.update_scene(dt);
                            tri_pick_demo.draw_scene();
                        }
