...
pub struct D3DApp {
    ...
    bounding_box_demo: Option<BoundingBoxDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let bounding_box_demo = BoundingBoxDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.bounding_box_demo = bounding_box_demo;
                ...
                if let Some(bounding_box_demo) = &d3d_app.bounding_box_demo {
                    bounding_box_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(bounding_box_demo) = &self.bounding_box_demo {
            bounding_box_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(bounding_box_demo) = &mut self.bounding_box_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Bounding Box Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(bounding_box_demo) = &mut self.bounding_box_demo {
                            bounding_box_demo.update_scene(dt);
                            bounding_box_demo.draw_scene();
                        }
