...
pub struct D3DApp {
    ...
    robot_arm_demo: Option<RobotArmDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let robot_arm_demo = RobotArmDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.robot_arm_demo = robot_arm_demo;
                ...
                if let Some(robot_arm_demo) = &d3d_app.robot_arm_demo {
                    robot_arm_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(robot_arm_demo) = &self.robot_arm_demo {
            robot_arm_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(robot_arm_demo) = &mut self.robot_arm_demo {
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Robot Arm Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(robot_arm_demo) = &mut self.robot_arm_demo {
                            robot_arm_demo.update_scene(dt);
                            robot_arm_demo.draw_scene();
                        }
