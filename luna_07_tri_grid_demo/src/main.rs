...
pub struct D3DApp {
    ...
    tri_grid_demo: Option<TriGridDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let tri_grid_demo = TriGridDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.tri_grid_demo = tri_grid_demo;
                ...
                if let Some(tri_grid_demo) = &d3d_app.tri_grid_demo {
                    tri_grid_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(tri_grid_demo) = &self.tri_grid_demo {
            tri_grid_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(tri_grid_demo) = &mut self.tri_grid_demo {
            tri_grid_demo.on_reset_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Grid Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(tri_grid_demo) = &mut self.tri_grid_demo {
                            tri_grid_demo.update_scene(dt);
                            tri_grid_demo.draw_scene();
                        }
