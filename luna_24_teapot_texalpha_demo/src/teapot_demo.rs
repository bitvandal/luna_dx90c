...
impl TeapotDemo {
    pub fn new(d3d_device: IDirect3DDevice9, d3d_pp: *const D3DPRESENT_PARAMETERS) -> Option<TeapotDemo> {
        ...
        HR!(D3DXCreateTextureFromFile(d3d_device.clone(), PSTR(b"luna_24_teapot_texalpha_demo/crate.jpg\0".as_ptr() as _), &mut crate_tex));

        let mut teapot_tex: *mut c_void = std::ptr::null_mut();
        HR!(D3DXCreateTextureFromFile(d3d_device.clone(), PSTR(b"luna_24_teapot_texalpha_demo/bricka.dds\0".as_ptr() as _), &mut teapot_tex));
    ...
    fn build_fx(d3d_device: IDirect3DDevice9) -> (LPD3DXEFFECT, D3DXHANDLE, D3DXHANDLE, D3DXHANDLE,
        ...
        HR!(D3DXCreateEffectFromFile(d3d_device, PSTR(b"luna_24_teapot_texalpha_demo/dirLightTex.fx\0".as_ptr() as _),
