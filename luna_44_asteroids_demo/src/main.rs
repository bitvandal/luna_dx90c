pub mod camera;
pub mod asteroids_demo;
pub mod terrain;
pub mod firework_psystem;
...
pub struct D3DApp {
    ...
    asteroids_demo: Option<AsteroidsDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let asteroids_demo =
                    AsteroidsDemo::new(d3d_app.get_main_wnd(),
                                     d3d_device.clone(),
                                     &d3d_app.d3d_pp, thread_rng());
                d3d_app.asteroids_demo = asteroids_demo;
                ...
                if let Some(asteroids_demo) = &d3d_app.asteroids_demo {
                    asteroids_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(asteroids_demo) = &mut self.asteroids_demo {
            asteroids_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Asteroids Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(asteroids_demo) = &mut self.asteroids_demo {
                            asteroids_demo.update_scene(dt);
                            asteroids_demo.draw_scene();
                        }
