...
impl FireWorkPSystem {
    pub fn new(base_path: &str, fx_name: &str, tech_name: &str, tex_name: &str,
        ...
        let mut initialized_particles: Vec<Particle> = Vec::new();
        for i in 0..max_num_particles as usize {
            let mut particle = psys.particles[i].clone();
            psys.init_particle(&mut particle);
            initialized_particles.push(particle)
        }
        psys.particles = initialized_particles;
    ...
    pub fn init_particle(&mut self, out: &mut Particle) {
        out.initial_time = 0.0;
        out.initial_size = get_random_float(&mut self.rng, 12.0, 15.0);
        out.life_time = 10.0;

        // Generate Random Direction
        let mut d = D3DXVECTOR3::default();
        get_random_vec(&mut self.rng, &mut d);

        // Compute velocity.
        let speed: f32 = get_random_float(&mut self.rng, 100.0, 150.0);
        D3DXVec3Scale(&mut out.initial_velocity, &d, speed);

        out.initial_color = D3DCOLOR_XRGB!(255, 255, 255);
        out.mass = get_random_float(&mut self.rng, 2.0, 4.0);

        let r: f32 = get_random_float(&mut self.rng, 0.0, 2.0);
        D3DXVec3Scale(&mut out.initial_pos, &d, r);
    }
