...
impl RainPSystem {
    ...
    pub fn init_particle(&mut self, out: &mut Particle) {
        unsafe {
            ...
            out.initial_pos.x += get_random_float(&mut self.rng, -100.0, 100.0);
            out.initial_pos.z += get_random_float(&mut self.rng, -100.0, 100.0);

            // Generate above the camera.
            out.initial_pos.y += get_random_float(&mut self.rng, 50.0, 55.0);

            out.initial_time = self.time;
            out.life_time = get_random_float(&mut self.rng, 2.0, 2.5);
            out.initial_color = D3DCOLOR_XRGB!(255, 255, 255);
            out.initial_size = get_random_float(&mut self.rng, 6.0, 7.0);

            // Give them an initial falling down velocity.
            out.initial_velocity.x = get_random_float(&mut self.rng, -1.5, 0.0);
            out.initial_velocity.y = get_random_float(&mut self.rng, -50.0, -45.0);
            out.initial_velocity.z = get_random_float(&mut self.rng, -0.5, 0.5);
        }
    }
