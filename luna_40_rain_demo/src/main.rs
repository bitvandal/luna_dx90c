    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                ...
                if let Some(fire_ring_demo) = &d3d_app.fire_ring_demo {
                    fire_ring_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&mut self) {
        if let Some(fire_ring_demo) = &mut self.fire_ring_demo {
            fire_ring_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Fire Ring Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(fire_ring_demo) = &mut self.fire_ring_demo {
                            fire_ring_demo.update_scene(dt);
                            fire_ring_demo.draw_scene();
                        }
