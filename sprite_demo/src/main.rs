    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let options = SpriteDemoOptions {
                    bullet_speed: 2500.0,
                    max_ship_speed: 1500.0,
                    ship_accel: 1000.0,
                    ship_drag: 0.85,
                };

                let sprite_demo = SpriteDemo::new(d3d_device.clone(), options);
                d3d_app.sprite_demo = sprite_demo;
                ...
                if let Some(sprite_demo) = &d3d_app.sprite_demo {
                    sprite_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(sprite_demo) = &self.sprite_demo {
            sprite_demo.on_lost_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Sprite Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(sprite_demo) = &mut self.sprite_demo {
                            sprite_demo.update_scene(dt);
                            sprite_demo.draw_scene();
                        }
