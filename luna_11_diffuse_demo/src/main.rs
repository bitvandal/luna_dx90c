...
pub struct D3DApp {
    ...
    diffuse_demo: Option<DiffuseDemo>,
}
    ...
    unsafe {
        ...
        if let Some(d3d_app) = &mut D3D_APP {
            if let Some(d3d_device) = D3D_DEVICE.clone() {
                let diffuse_demo = DiffuseDemo::new(d3d_device.clone(), &d3d_app.d3d_pp);
                d3d_app.diffuse_demo = diffuse_demo;
                ...
                if let Some(diffuse_demo) = &d3d_app.diffuse_demo {
                    diffuse_demo.release_com_objects();
                }
...
impl D3DApp {
    ...
    fn on_lost_device(&self) {
        if let Some(diffuse_demo) = &self.diffuse_demo {
            diffuse_demo.on_lost_device();
        }
    }

    fn on_reset_device(&mut self) {
        if let Some(diffuse_demo) = &mut self.diffuse_demo {
            diffuse_demo.on_reset_device();
        }
    }
    ...
    fn init_main_window(&mut self) {
        unsafe {
            ...
            let main_wnd_caption = PSTR(b"Diffuse Demo\0".as_ptr() as _);
            ...
            while msg.message != WM_QUIT {
                ...
                if PeekMessageA(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    ...
                    if !self.device_is_lost() {
                        ...
                        if let Some(diffuse_demo) = &mut self.diffuse_demo {
                            diffuse_demo.update_scene(dt);
                            diffuse_demo.draw_scene();
                        }
